...
function Resposta_Frequencia_Hs4(Hs,logw1,logw2)
 subplot(221)
 logw=linspace(logw1,logw2,1024);
 w=10.^logw;
 H=horner(Hs,%i*w)
 plot(logw,20*log10(abs(H)),'r')
 gca().data_bounds=[logw1,-100,0;logw2,20,0]
 title("|H(jw)|dB")
 subplot(223)
 plot(logw,phasemag(H,'c'));
 title("Fase em Graus")
 subplot(222)
 w=linspace(10.^logw1,10.^logw2,100)
 H=horner(Hs,%i*w)
 plot(w,abs(H))
 gca().data_bounds=[min(w),0,0;max(w)*.5,max(abs(H))*1.1,0]
 title("|H(jw)|")
 subplot(224)
 plot_polos_zeros(1,Hs)
endfunction
